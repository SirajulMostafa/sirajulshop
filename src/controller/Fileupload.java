...
public class Fileupload extends HttpServlet {
                    ...
                    for (FileItem item : multiparts) {
                       if (!item.isFormField()) {                
                           ...
                           //  String key = UUID.randomUUID().toString();
                        	...
                        	if(item.getFieldName().equals("imgID")){
                        		imgId = item.getString();			
                            }
                        }                                    
                    }
    					...
    					"root",null); 
    				    String sql = "update products set small_picture=? where id=? ";
    					
    				    PreparedStatement ps=con.prepareStatement(sql); 
    				    ps.setString(1,image); 
						ps.setString(2,imgId);  
    				    int i=ps.executeUpdate();  
    				     System.out.println(i+" records affected"+request.getParameter("imgID"));  
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		 ...
		 processRequest(request, response);
		/*String imgId=	request.getParameter("imgI");
    if (ServletFileUpload.isMultipartContent(request)) {     
            try { 
                ...
                for (FileItem item : multiparts) {
                   if (!item.isFormField()) {                
			        ...
			        ps.setString(2,imgId);  
				    int i=ps.executeUpdate();  
				    System.out.println(i+" records affected"+imgId);  
